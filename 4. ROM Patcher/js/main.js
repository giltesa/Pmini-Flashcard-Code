...
// ===== Patch the games to remove sleep mode (Thank you to zoranc, zwenergy & YasaSheep) =====
...
function patchGameResumeBehavior(entry) {
  const PATCH_DATA = {
    ...
    "MSDE": [ { offset: 0x2A88, from: 0x42, to: 0x48 },    // Breeder US
              { offset: 0x2B6B, from: 0x44, to: 0x48 },
    ],
  ...
  const patches = PATCH_DATA[entry.gameCode];

  if (!patches) return entry.bytes; // No patches for this game
